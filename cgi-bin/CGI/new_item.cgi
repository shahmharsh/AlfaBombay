...
my $query = "INSERT INTO products VALUES(?,?,?,?,?,?,?,?,?)";
#my $query = "INSERT INTO products VALUES('$sku', '$category', '$vendor', '$manufacturer_id', '$description', '$features', $cost, $retail, '$image')";

my $sth = $dbh->prepare($query) or die "Couldn't prepare";
$sth->execute($sku, $category, $vendor, $manufacturer_id, $description, $features, $cost, $retail, $image);
